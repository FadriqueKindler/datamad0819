...
def getPlayers():
    players = pd.read_csv("../pipelines-project/DATA/nba-players-stats/players.csv")
...
def get_player_data():
    player_data = pd.read_csv("../pipelines-project/DATA/nba-players-stats/players.csv")
...
def get_season_stats():
    season_stats = pd.read_csv("../pipelines-project/DATA/nba-players-stats/Seasons_Stats.csv")
...
def merging_dfs(df_players,df_player_data,df_season_stats):
    ...
    complete_df.drop(columns=["G","TS%","FTr","OWS","DWS","WS","FG","FGA","FG%","2P","2PA",
                            "2P%","eFG%","FT","FTA","AST","PF","PTS","FT%","GS","3PAr","TOV%",
                            "USG%","3P","3P%","3PA","TOV","BLK","STL","TRB","DRB","ORB","VORP",
                            "BPM","DBPM","OBPM","WS/48","BLK%","STL%","AST%","TRB%","DRB%","ORB%",
                            "MP","PER"],inplace=True)
...
def wrangle_webscraping(player_df,col_df):
    ...
    player_df = player_df.rename(columns={0: "Rank", 1: "Player",2: "Points",3: "Seasons",4: "Games",
...
def final_NBA50_df(player_finalstats_df, top_player_df):
    ...
    master_player_df[["Rank", "Points","FGM","FGA","FTM","FTA",
                      "3PM","3PA","Games","Seasons"]] = master_player_df[["Rank", "Points",
                                                                          "FGM","FGA","FTM","FTA",
                                                                          "3PM","3PA","Games",
                                                                          "Seasons"]].apply(pd.to_numeric)
